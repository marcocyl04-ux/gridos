...
# Loads plugins/<slug>/plugin.yaml files. Runs formulas, agents, templates
# through YAML only — no executable code. Safe to run at startup.
...
# Register declarative agents alongside Python plugin agents
...
# Install declarative formulas into the kernel now that engine is loaded
try:
...
@app.post("/agent/write/graph", response_model=WriteResponse)
async def agent_write_graph(
    req: NodeGraphRequest,
    k: GridOSKernel = Depends(current_kernel_dep),
):
    """Execute LLM intent through typed node graph."""
    try:
...
async def import_file_endpoint(
    file: UploadFile = File(...),
    has_header: bool = Query(default=True, description="Whether the first row is headers"),
    sheet_name: Optional[str] = Query(None, description="For Excel: which sheet to import"),
    target_sheet: str = Query("Sheet1", description="Which GridOS sheet to import into"),
    clear_existing: bool = Query(True, description="Clear target sheet before import"),
    k: GridOSKernel = Depends(current_kernel_dep),
):
    """Import CSV or Excel file into the current workbook."""
    import tempfile
    import shutil
    from io import BytesIO
    ...
    try:
        # Write to a temp file so import_file (which expects a path) can read it
        ...
        for s in result.sheets:
            ...
            for a1_ref, icell in s.cells.items():
                sheet_cells[a1_ref] = {
                    "value": icell.value,
                    "formula": icell.formula,
                    "locked": False,
                    "datatype": icell.datatype,
                    "agent_owner": "User",
                }
            sheets_data.append({
                "name": s.name,
        ...
        return {
            "status": "Success",
            "sheets_imported": len(result.sheets),
...
async def import_file_endpoint(
    file: UploadFile = File(...),
    has_header: bool = Query(True, description="Whether the first row is headers"),
    sheet_name: Optional[str] = Query(None, description="For Excel: which sheet to import"),
    target_sheet: str = Query("Sheet1", description="Which GridOS sheet to import into"),
    clear_existing: bool = Query(True, description="Clear target sheet before import"),
):
